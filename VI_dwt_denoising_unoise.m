...
mix = [2 1];
sinunoise = mix(1)*swave + mix(2)*unoise;
mwunoise = mix(1)*mwavelet + mix(2)*unoise;
bwunoise = mix(1)*boxwave + 0.5*mix(2)*unoise;
...
[c, l] = wavedec(sinunoise, nlevel, wname);

% Original wavelet coefficients
figure, subplot(nlevel+2, 1, 1), plot(t, sinunoise), ylabel('original'),title('DWT coefficients- sine waves + normal noise');
for ilevel = 1:nlevel
...
figure, subplot(nlevel+2, 1, 1), plot(t, sinunoise), ylabel('original'), title('Denoised DWT coefficients- sine waves + normal noise');
for ilevel = 1:nlevel
...
% Result of denoising
figure, subplot(211), plot(t, sinunoise), xlim([0 1]), ylabel('amp'), title('Sinewave + normal noise'),  grid minor;
...
% Multilevel 1-D wavelet decomposition - Mallat
[c, l] = wavedec(mwunoise, nlevel, wname);

% Original wavelet coefficients
figure, subplot(nlevel+2, 1, 1), plot(t, mwunoise), ylabel('original'),title('DWT coefficients- morlet wave + normal noise');
for ilevel = 1:nlevel
    subplot(nlevel+2, 1, ilevel+1), plot(detcoef(c, l, ilevel)), ylabel(['d', num2str(ilevel)]),
    title([num2str(Fs/2^(ilevel+1)), 'Hz - ', num2str(Fs/2^ilevel), 'Hz']);
end
...
figure, subplot(nlevel+2, 1, 1), plot(t, mwunoise), ylabel('original'),title('Denoised DWT coefficients- morlet waves + normal noise');
for ilevel = 1:nlevel
    subplot(nlevel+2, 1, ilevel+1), plot(detcoef(denoise_c, l, ilevel)), ylabel(['d', num2str(ilevel)]),
    title([num2str(Fs/2^(ilevel+1)), 'Hz - ', num2str(Fs/2^ilevel), 'Hz']);
end
...
% Result of denoising
figure, subplot(211), plot(t, mwunoise), xlim([0 9]), ylabel('amp'), title('Morlet wave + normal noise');
...
% Multilevel 1-D wavelet decomposition - Mallat
[c, l] = wavedec(bwunoise, nlevel, wname);

% Original wavelet coefficients
figure, subplot(nlevel+2, 1, 1), plot(t, bwunoise), ylabel('original'),title('DWT coefficients- box wave + normal noise');
for ilevel = 1:nlevel
    subplot(nlevel+2, 1, ilevel+1), plot(detcoef(c, l, ilevel)), ylabel(['d', num2str(ilevel)]),
    title([num2str(Fs/2^(ilevel+1)), 'Hz - ', num2str(Fs/2^ilevel), 'Hz']);
end
...
figure, subplot(nlevel+2, 1, 1), plot(t, bwunoise), ylabel('original'),title('Denoised DWT coefficients- sine waves + normal noise');
for ilevel = 1:nlevel
    subplot(nlevel+2, 1, ilevel+1), plot(detcoef(denoise_c, l, ilevel)), ylabel(['d', num2str(ilevel)]),
    title([num2str(Fs/2^(ilevel+1)), 'Hz - ', num2str(Fs/2^ilevel), 'Hz']);
end
...
% Result of denoising
figure, subplot(211), plot(t, bwunoise), xlim([0 1]), ylabel('amp'), title('Boxwave + normal noise');
