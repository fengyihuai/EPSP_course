function denoised_eeg = dwtStEogDenoise(eeg, Fs, wname, nlevel, denoised_levels, thr_type)
...
for ichannel = 1:num_channels
    ...
    figure, subplot(l_l+2, 1, 1), plot(t, eeg(ichannel, :)), ylabel('original'), title(['ST-', thr_type, '-Original & denoised DWT coefficients - channel', num2str(ichannel)]);
    for ilevel = denoised_levels
